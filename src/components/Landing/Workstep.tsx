...
const Card = ({ title, description, src, color, i }: CardProps) => {
  ...
  return (
    <div
      ref={container}
      className="h-screen flex items-center justify-center sticky sm:top-10 top-20 sm:px-12 px-6 my-10"
    >
        <div
          className="relative flex justify-center items-center h-[400px] w-full rounded-xl p-12 opacity-100"
          style={{
            backgroundColor: color,
            transform: `translateY(calc(-5vh + ${i * 25}px))`,
          }}
        >
...
export default function Timeline() {
  return (
    <div className="flex flex-col lg:flex-row items-start justify-between relative w-full sm:py-10 py-5 sm:px-12 px-6 max-w-[1440px] mx-auto">
      ...
      <div className="lg:w-1/2 sticky top-0 h-screen flex flex-col justify-center">
        ...
        <div className="flex justify-start">
            <Link
                ...
                className="bg-[#122461] rounded-[8px] px-6 py-3 flex justify-center items-center gap-2 group transition-all duration-300 cursor-pointer whitespace-nowrap mt-8"
            >
